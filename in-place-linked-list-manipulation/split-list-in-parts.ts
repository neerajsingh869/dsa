 ...
 * the linked list into k consecutive linked list parts.
  ...
  should have a size differing by more than one. This may lead to 
...
function splitListToParts(
  ...
  const baseChunkSize = Math.floor(length / k);
  let extraNodes = length % k;
  const result: Array<ListNode | null> = [];
  current = head;

  for (let i = 0; i < k; i++) {
    const currentChunkHead = current;
    const currentChunkSize = baseChunkSize + (i < extraNodes ? 1 : 0);

    for (let j = 0; j < currentChunkSize - 1 && current != null; j++) {
      current = current.next;
    }

    if (current != null) {
      const nextChunkHead = current.next;
      current.next = null;
      current = nextChunkHead;
    }

    result.push(currentChunkHead);
  }
